...
def set_design():
    ...
    [data-testid="stSidebar"] input, [data-testid="stSidebar"] select {{
        color: #000000 !important; font-weight: 900 !important; font-size: 18px !important;
    }}
    ...
    [data-testid="stSidebar"] h1, [data-testid="stSidebar"] h2, [data-testid="stSidebar"] h3 {{
        color: #FFFFFF !important; font-size: 35px !important; border-bottom: 2px solid white;
    }}
    </style>
...
if os.path.exists(DATA_FILE):
    ...
    # Proper column order for the CSV
...
with st.sidebar:
    ...
    u_round = st.number_input("Round No.", min_value=1, step=1) # ROUND Fix: Ab aap khud Round daal sakte hain
    weight = st.number_input("Weight (KG)", min_value=0.0)
    rate = st.number_input("Rate", min_value=0.0, format="%.4f")
    diesel = st.number_input("Diesel", min_value=0.0)
    d_pay = st.number_input("Driver Kharcha", min_value=0.0) # Driver Kharcha input
    ...
    if st.button("💾 SAVE RECORD"):
        ...
        new_row = {
            ...
            "DRIVER_NAME": d_name,
            "ROUND": u_round, # Ab ye user wala Round save karega
            "WEIGHT": weight, 
            "RATE": rate, 
            "KAMAI": round(kamai, 2),
            "DIESEL": diesel, 
...
# Column Order Fix: DIESEL ke baad DRIVER_EXP
